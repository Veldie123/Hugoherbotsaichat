...
export function StickyHeader({ currentPage = "landing", navigate }: StickyHeaderProps) {
  ...
  // Global navigation items (visible on all pages - desktop only)
  const navItems = [
    { label: "Pricing", page: "pricing" as Page },
  ];

  // Mobile-only navigation (visible on all pages)
  const mobileNavItems = [
    { label: "Over Hugo", page: "about" as Page },
    { label: "Prijzen", page: "pricing" as Page },
    { label: "Probeer de app", page: "preview" as Page },
  ];

  // Navigate to a section on landing page, or navigate to landing + scroll
  const handleLandingNavigation = (sectionId: string) => {
    console.log("🔍 handleLandingNavigation called with:", sectionId);
    
    // Close menu first
    setMobileMenuOpen(false);
    
    if (currentPage === "landing") {
      // Already on landing, just scroll after menu closes
      setTimeout(() => {
      ...
      // Navigate to landing first, then scroll after a delay
      navigate("landing");
      setTimeout(() => {
        const element = document.getElementById(sectionId);
  ...
  const landingAnchors = [
    ...
    { label: "EPIC Methode", id: "epic-methode" },
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {mobileMenuOpen && (
          <div className="md:hidden fixed inset-0 top-20 sm:top-24 bg-white z-50 overflow-auto">
            <div className="flex flex-col h-full">
              {/* Navigation Items */}
              <nav className="flex flex-col p-6 space-y-1">
                {/* Landing Page Anchors - Always show on all pages */}
                {landingAnchors.map((anchor) => (
                  <button
                    key={anchor.id}
                    onClick={() => handleLandingNavigation(anchor.id)}
                    className="px-4 py-4 text-left text-[18px] leading-[26px] font-[400] text-hh-text hover:text-hh-ink hover:bg-hh-ui-50 rounded-lg transition-colors"
                  >
                    {anchor.label}
                  </button>
                ))}
