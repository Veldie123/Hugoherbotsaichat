...
export function UserProvider({ children }: { children: ReactNode }) {
    ...
    try {
      ...
      if (storedWorkspace) {
        ...
        try {
          ...
          console.error('❌ Network error fetching workspaces:', fetchError);
          // Continue without workspace - user can still use the app
        }
      }
  ...
  useEffect(() => {
    ...
    const subscription = auth.onAuthStateChange((event, newSession) => {
      ...
      // Ignore token refresh events to prevent duplicate calls
      if (event === 'SIGNED_IN') {
        loadUserData();
