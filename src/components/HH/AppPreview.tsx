...
import { TalkToHugoAI } from "./TalkToHugoAI";
import { SignupModal } from "./SignupModal";
import { Badge } from "../ui/badge";
import { Button } from "../ui/button";
import { Sparkles, ArrowRight } from "lucide-react";

type PreviewPage = "dashboard" | "roleplay" | "roleplayvideo" | "roleplaychat" | "library" | "builder" | "mysessions" | "videos" | "live" | "team" | "analytics" | "settings" | "digitalcoaching" | "conversationanalysis" | "talk-to-hugo";
...
export function AppPreview({ navigate }: AppPreviewProps) {
  ...
  const handlePreviewNavigate = (page: string) => {
    ...
    // Handle coaching with query parameter (e.g., "coaching?technique=talk-to-hugo-ai")
    if (page.startsWith("coaching?")) {
      setCurrentPreviewPage("digitalcoaching");
      window.scrollTo(0, 0);
      return;
    }
    ...
    <div className="relative">
      ...
      <div>
        {currentPreviewPage === "dashboard" && (
          <Dashboard hasData={true} navigate={handlePreviewNavigate} isAdmin={false} />
        )}
        {currentPreviewPage === "roleplay" && (
          <RolePlay navigate={handlePreviewNavigate} isAdmin={false} />
        )}
        {currentPreviewPage === "roleplayvideo" && (
          <RolePlay navigate={handlePreviewNavigate} isAdmin={false} />
        )}
        {currentPreviewPage === "roleplaychat" && (
          <RolePlayChat navigate={handlePreviewNavigate} isAdmin={false} />
        )}
        {currentPreviewPage === "library" && (
          <OverviewProgress navigate={handlePreviewNavigate} isAdmin={false} />
        )}
        {currentPreviewPage === "builder" && (
          <ScenarioBuilder navigate={handlePreviewNavigate} isAdmin={false} />
        )}
        {currentPreviewPage === "mysessions" && (
          <MySessions navigate={handlePreviewNavigate} isAdmin={false} />
        )}
        {currentPreviewPage === "videos" && (
          <VideoLibrary navigate={handlePreviewNavigate} isAdmin={false} />
        )}
        {currentPreviewPage === "live" && (
          <LiveCoaching navigate={handlePreviewNavigate} isAdmin={false} />
        )}
        {currentPreviewPage === "team" && (
          <TeamSessions navigate={handlePreviewNavigate} isAdmin={false} />
        )}
        {currentPreviewPage === "analytics" && (
          <Analytics navigate={handlePreviewNavigate} isAdmin={false} />
        )}
        {currentPreviewPage === "settings" && (
          <Settings navigate={handlePreviewNavigate} isAdmin={false} />
        )}
        {currentPreviewPage === "digitalcoaching" && (
          <DigitalCoaching navigate={handlePreviewNavigate} isAdmin={false} />
        )}
        {currentPreviewPage === "conversationanalysis" && (
          <ConversationAnalysis navigate={handlePreviewNavigate} isAdmin={false} />
        )}
        {currentPreviewPage === "talk-to-hugo" && (
          <TalkToHugoAI navigate={handlePreviewNavigate} isAdmin={false} />
        )}
      </div>
