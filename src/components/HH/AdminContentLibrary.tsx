...
export function AdminContentLibrary({ navigate }: AdminContentLibraryProps) {
  ...
  const stats = [
    { label: "Total Content", value: allContent.length, icon: Library, color: "purple", trend: "up", change: "+10%" },
    { label: "Video's", value: allContent.filter((c) => c.type === "video").length, icon: Video, color: "purple", trend: "up", change: "+5%" },
  ...
  const filteredContent = allContent.filter((content) => {
    ...
    const matchesFase = filterFase === "all" || content.fase === filterFase;
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          {stats.map((stat) => {
            ...
            const bgColor = stat.color === "purple" ? "bg-purple-600/10" : 
                           stat.color === "blue" ? "bg-blue-600/10" :
                           stat.color === "red" ? "bg-red-600/10" : "bg-green-600/10";
            const iconColor = stat.color === "purple" ? "text-purple-600" : 
