      ...
      <div className="p-4 sm:p-6 lg:p-8 space-y-6 sm:space-y-8">
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-hh-text">Jouw analyses</h2>
            <p className="text-[14px] leading-[20px] text-hh-muted">
              {analyses.length} gesprekken
            </p>
          </div>
          ...
          <div className="space-y-3">
            {analyses.map((analysis) => (
              <Card
                key={analysis.id}
                className="p-5 rounded-[16px] border-hh-border hover:border-hh-primary/30 transition-all"
              >
                <div className="flex items-start justify-between gap-6">
                  {/* Info */}
                  <div className="flex-1 min-w-0 space-y-3">
                    <div>
                      <h3 className="text-hh-text mb-1.5">{analysis.title}</h3>
                    ...
                    {analysis.status === "completed" && (
                      <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-[14px] leading-[20px]">
                        <div className="flex items-center gap-2">
                          <span className="text-hh-muted">Score:</span>
                          <span className="text-hh-text font-medium">
                            {analysis.overallScore}%
                          </span>
                          {analysis.scoreDelta === "up" && (
                            <TrendingUp className="w-4 h-4 text-hh-success" />
                          )}
                          {analysis.scoreDelta === "down" && (
                            <TrendingDown className="w-4 h-4 text-hh-destructive" />
                          )}
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="text-hh-muted">Techniek:</span>
                          <span className="text-hh-text">{analysis.topTechnique}</span>
                        </div>
                        <Badge variant="outline" className="text-[12px]">{analysis.phase}</Badge>
                      </div>
                    )}
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    {analysis.status === "completed" && (
                      <Button
                        ...
                        size="sm"
                        onClick={() =>
                          navigate?.("analysis-results", { id: analysis.id })
                        }
                        className="gap-2"
                      >
                    ...
                    <Button
                      variant="ghost"
                      size="icon"
                      className="h-9 w-9 text-hh-muted hover:text-hh-destructive"
                    >
